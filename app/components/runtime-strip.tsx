...
export function RuntimeStrip() {
  ...
  useEffect(() => { fetch('/api/admin/runtime/overview', { cache: 'no-store' }).then(async (response) => response.ok ? await response.json() as RuntimeOverview : null).then(setOverview).catch(() => undefined); }, []);
