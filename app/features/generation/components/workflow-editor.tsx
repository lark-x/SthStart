      ...
      <CardContent className="space-y-3">
        <form key={selectedWorkflowId} id="workflow-version-form" onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-4">
            <div><label htmlFor="generation-input-schema" className="mb-1 block text-sm font-semibold">输入结构</label><Textarea id="generation-input-schema" name="version-input-schema" rows={8} className="font-mono text-sm" defaultValue={JSON.stringify({ prompt: { type: 'string' } }, null, 2)} /></div>
            <div><label htmlFor="generation-input-capabilities" className="mb-1 block text-sm font-semibold">媒体输入能力</label><Textarea id="generation-input-capabilities" name="version-input-capabilities" rows={8} className="font-mono text-sm" defaultValue="{}" /></div>
            <div><label htmlFor="generation-node-bindings" className="mb-1 block text-sm font-semibold">节点绑定</label><Textarea id="generation-node-bindings" name="version-node-bindings" rows={8} className="font-mono text-sm" defaultValue={JSON.stringify({ prompt: ['1', 'inputs', 'text'] }, null, 2)} /></div>
            <div><label htmlFor="generation-definition" className="mb-1 block text-sm font-semibold">ComfyUI API JSON</label><Textarea id="generation-definition" name="version-definition" rows={8} className="font-mono text-sm" defaultValue={SAMPLE_DEFINITION} /></div>
          </div>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            ...
            <Textarea aria-label="输出结构" placeholder="输出结构 JSON" name="version-output-schema" rows={1} className="font-mono text-sm" defaultValue="{}" />
          </div>
        </form>
      </CardContent>
      <CardFooter>
