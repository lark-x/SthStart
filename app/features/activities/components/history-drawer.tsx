      ...
      <div className="p-1 space-y-4">
        ...
        >
          ...
          <Button
            ...
            className="h-8 text-xs bg-[#e45d35] hover:bg-[#b83b1b] text-white flex items-center gap-1 flex-shrink-0"
          >
        ...
        <div className="space-y-2">
          <span className="text-xs font-semibold text-[#18201d]">已保存检查点 ({checkpoints.length})</span>

          {checkpointsLoading ? (
            <div className="text-xs text-[#68716d] py-6 text-center">加载检查点列表中…</div>
          ) : checkpoints.length === 0 ? (
            <div className="py-8 text-center text-xs text-[#68716d] bg-stone-50 rounded border border-stone-200">
            ...
            checkpoints.map((cp) => (
              <div
                key={cp.id}
                className="p-3 rounded-lg bg-white border border-stone-200 text-xs space-y-2 shadow-2xs"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-[#18201d]">{cp.name}</span>
                    <Badge variant="outline" className="text-[10px] bg-stone-100 font-mono">
                      v{cp.headVersion}
                    </Badge>
                  </div>
                  ...
                  <Button
                    ...
                    className="text-xs h-7 text-[#e45d35] hover:bg-[#e45d35]/10 flex items-center gap-1"
                  >
                ...
                <div className="text-[10px] text-stone-500 font-mono flex flex-wrap gap-x-3 gap-y-0.5">
        ...
        <div className="space-y-2 pt-2 border-t border-[rgb(24_32_29/10%)]">
          <span className="text-xs font-semibold text-[#18201d]">内容版本历史 ({revisions.length})</span>
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-2 rounded text-xs border ${
                    isCurrent ? 'bg-[#e45d35]/5 border-[#e45d35]/40' : 'bg-white border-stone-200'
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-[11px] text-[#18201d]">{rev.id.slice(0, 12)}…</span>
                    {isCurrent && (
