...
test('fresh databases record an explicit migration baseline', () => {
  ...
  assert.equal(migrations[25].name, 'activity-media-batches');
  assert.equal(migrations[26].name, 'activity-reusable-presets');
...
test('version one databases migrate existing LLM profiles to text capability', () => {
  ...
  assert.equal(migrated.connection.prepare('SELECT MAX(version) version FROM schema_migrations').get()!.version, 28);
  migrated.close();
});
