...
test('health and capabilities expose stable v1 contracts', async () => {
  ...
  assert.deepEqual(capabilities.json().modules.map((item: { id: string }) => item.id), [
    'app-registry', 'llm-gateway', 'vector-service', 'image-service', 'persona-catalog', 'creative-notebook', 'narrative-archive', 'runtime-manager', 'artifact-service', 'generation-core', 'activity-studio',
  ]);
